...
def create_complex_pbr_material() -> bpy.types.Material:
    ...
    coordinates = _node(nodes, "ShaderNodeTexCoord", "Texture Coordinates", (-1400, 260))
    mapping = _node(nodes, "ShaderNodeMapping", "UV Mapping", (-1150, 260))
...
def create_mix_material() -> bpy.types.Material:
    material = bpy.data.materials.new("OCTANIFY_FIXTURE_mix_material")
    material.use_nodes = True
    nodes = material.node_tree.nodes
    links = material.node_tree.links
    nodes.clear()
    ...
    output = _node(nodes, "ShaderNodeOutputMaterial", "Material Output", (700, 0))
    mix = _node(nodes, "ShaderNodeMixShader", "Cycles Mix Shader", (400, 0))
    first = _node(nodes, "ShaderNodeBsdfPrincipled", "First Principled", (50, 150))
    second = _node(nodes, "ShaderNodeBsdfPrincipled", "Second Principled", (50, -150))
    first.inputs["Base Color"].default_value = (0.8, 0.05, 0.02, 1.0)
    second.inputs["Base Color"].default_value = (0.02, 0.05, 0.8, 1.0)
    mix.inputs[0].default_value = 0.25
    links.new(first.outputs["BSDF"], mix.inputs[1])
    links.new(second.outputs["BSDF"], mix.inputs[2])
    links.new(mix.outputs["Shader"], output.inputs["Surface"])
    return material


def create_multiple_outputs_material() -> bpy.types.Material:
    """Exercise active-output preservation with two authored render paths."""
    material = bpy.data.materials.new("OCTANIFY_FIXTURE_multiple_outputs")
    material.use_nodes = True
    nodes = material.node_tree.nodes
    links = material.node_tree.links
    nodes.clear()

    first = _node(nodes, "ShaderNodeBsdfPrincipled", "Primary Principled", (0, 160))
    second = _node(nodes, "ShaderNodeBsdfPrincipled", "Secondary Principled", (0, -160))
    first.inputs["Base Color"].default_value = (0.8, 0.1, 0.05, 1.0)
    second.inputs["Base Color"].default_value = (0.05, 0.2, 0.8, 1.0)
    primary_output = _node(
        nodes, "ShaderNodeOutputMaterial", "Primary Output", (400, 160)
    )
    secondary_output = _node(
        nodes, "ShaderNodeOutputMaterial", "Secondary Output", (400, -160)
    )
    links.new(first.outputs["BSDF"], primary_output.inputs["Surface"])
    links.new(second.outputs["BSDF"], secondary_output.inputs["Surface"])
    primary_output.target = "EEVEE"
    secondary_output.target = "CYCLES"
    # The globally active output belongs to another renderer. Octanify must
    # still follow Blender's explicit Cycles output when choosing a branch.
    primary_output.is_active_output = True
    return material
...
def create_fixture_scene() -> None:
    ...
    bpy.ops.mesh.primitive_cube_add(location=(2.5, 2.5, 0))
    mix_object = bpy.context.object
    mix_object.name = "OCTANIFY_FIXTURE_mix_material_object"
    mix_object.data.materials.append(create_mix_material())

    bpy.ops.mesh.primitive_cube_add(location=(4.5, 2.5, 0))
    outputs_object = bpy.context.object
    outputs_object.name = "OCTANIFY_FIXTURE_multiple_outputs_object"
    outputs_object.data.materials.append(create_multiple_outputs_material())
