...
Octane Glossy 31.10. Non-zero Diffuse Roughness selects Oren-Nayar because
...
Cinema 4D frequency correction for Blender 5.2 + Octane 31.10. Mapping
...
The installed Octane 31.10 node exposes the required noise families and a stable
...
This setting was verified in Blender 5.2 + Octane 31.10 by asserting the exact
...
The prominent **Keep Original Cycles Nodes** conversion option is enabled by
default. Disabling it removes the tagged Cycles nodes from each successfully
converted material after the Octane graph has been built and validated; failed
conversions retain their source graph. Light and World source branches continue
to be preserved for scene-domain reconversion and source-signature checks.

## Validation

- The post-fix A/B probe differs from a fresh node only for intended Cycles
  values and the intentional `GGX` BSDF selection.
- 157 Python regression tests cover Standard Surface, Universal, and Glossy
...
- Blender 5.2 with Octane 31.10 converts ten fixture materials with zero failed
  links, including modern Standard Surface/Mix nodes, active multiple outputs,
  and split Projection/UV Transform.
- Blender 5.2 with Octane 31.10 runtime checks also verify Composite Texture
  ...
  arrangement, one-level and three-level shader-group conversion through the
  operator, disabled custom coloring, preserved Cycles Light/World branches,
  Octane active-output resolution, the default checked Cycles-preservation
  state, unchecked automatic cleanup, and repeated add-on register/unregister
  cycles without leaked RNA properties.
...
`tools/blender_validate_glossy_material.py`, and
`tools/blender_validate_node_groups.py`. Registration lifecycle checks are in
`tools/blender_validate_addon_lifecycle.py`.
